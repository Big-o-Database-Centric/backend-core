...
export class SqlserverProvisioner extends BaseDockerProvisioner {
  ...
  async provision(input: ProvisioningInput) {
    ...
    const port = await this.start(input, ['ACCEPT_EULA=Y', `MSSQL_SA_PASSWORD=${saPassword}`], '/var/opt/mssql');
    await this.docker.waitForCommand(this.containerName(input.instanceId), ['/opt/mssql-tools18/bin/sqlcmd', '-C', '-S', 'localhost', '-U', 'sa', '-P', saPassword, '-Q', 'SELECT 1']);
